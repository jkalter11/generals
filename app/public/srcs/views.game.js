...
TGO.Views.gameView = (function() {
    ...
    function init() {
        ...
        viewCheatSheetButton.on('click', function(e) {
            e.stopPropagation();
            TGO.Views.utils.openSmallWindow('/help', 'TGO');
        });
    ...
    function onPlayerJoined() {
        ...
        if (game.isCreated) {
            setGameMessage('<span class="highlight">%s</span> connected. ARRANGE your pieces.', game.opponentName);
        } else {
            setGameMessage('WELCOME <span class="highlight">%s</span>! ARRANGE your game pieces.', game.playerName);
        }
    ...
    function initGameBoardPositions() {
        ...
        if (game.isCreated) {
            ...
            while (row > 0) {
                ...
                if (TGO.debug) {
                    gameBoardNumbers.find('tr:nth-child(' + row + ') td:nth-child(' + column + ')').text(position);
                }
                if (row > 5) {
